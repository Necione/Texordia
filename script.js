document.addEventListener("DOMContentLoaded", function () {
    ...
    function extractItemNameFromInput(input) {
      ...
      function handleHunting() {
        ...
        if (currentTime - lastHuntTime < 60000) { // 60000 milliseconds = 1 minute
    ...
    function attemptToPurchaseItem(itemName) {
      const item = shopItems.items.find((item) => item.name.toLowerCase() === itemName.toLowerCase());
      if (item) {
    ...
    function showInventory() {
      let inventoryDisplay = userInventory.length > 0 ? userInventory.join(", ") : "No items";
